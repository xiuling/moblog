...
<div class="contents" >
	...
	<?php
		...
		if($cursor){
			echo '<form action="update.php?action=changeabout&id='.$cursor['_id'].'" method="post"> ';
			echo '<table>';
			echo '<tr><td>Title</td><td><input type="text" name="title" value="'.$cursor['title'].'" size="30" /></td></tr>';
			echo '<tr><td>Content</td><td><textarea name="text" rows="25" cols="34.5">'. $cursor['text'] .'</textarea></td></tr>';
			echo '</table>';
		}
		else{
